...
class Detection3DArraySubscriber(Node):
    ...
    def lidar_pose_callback(self, msg):
        ...
        self.frame_id = msg.transform.rotation.w
    ...
    def box_data_callback(self, msg):
        ...
        pcd_time = self.get_clock().now().to_msg()
        print("pcd visualize time :", (pcd_time.sec + pcd_time.nanosec * 1e-9) - (start_time.sec + start_time.nanosec * 1e-9))
        ...
        ego_box_time = self.get_clock().now().to_msg()
        print("ego bbox visualize time :",
              (ego_box_time.sec + ego_box_time.nanosec * 1e-9) - (start_time.sec + start_time.nanosec * 1e-9))
        ...
        if len(self.ego_lidar_pose_msgs_buff) == 0:
            ...
            ego_frame_id = ego_lidar_pose_msg.transform.rotation.w
        ...
        if 1 == 1:
            ...
            print("cav bbox visualize time :",
                  (stop_time.sec + stop_time.nanosec * 1e-9) - (ego_box_time.sec + ego_box_time.nanosec * 1e-9))

            # frame_info是包含当前帧信息的字典，包括帧id，sec，nanosec
            frame_info = {"id": self.frame_id, "start_sec": start_time.sec + start_time.nanosec / 1e9,
                          "stop_sec": stop_time.sec + stop_time.nanosec / 1e9, "ego_frame_id": ego_frame_id}

            print("Fusion delay:      ", stop_time.sec + stop_time.nanosec / 1e9 - (start_time.sec + start_time.nanosec / 1e9))

            def save_frame_info(frame_info):
                # 打开文件，并以“a”模式追加写入
                with open("frame_info_fusion.txt", 'a') as file:
                    frame_id = frame_info["id"]
                    start_sec = "{:.9f}".format(frame_info["start_sec"])
                    stop_sec = "{:.9f}".format(frame_info["stop_sec"])
                    ego_frame_id = frame_info["ego_frame_id"]

                    # 将帧信息写入文件，并在每个帧信息后面添加换行符
                    file.write(f"Frame ID: {frame_id}, start_sec: {start_sec}, stop_sec:{stop_sec}, ego_frame_id:{ego_frame_id}\n")
